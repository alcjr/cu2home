...
def _filtered_properties(request):
    ...
    if form.is_valid():
        ...
        # Búsqueda textual
        # NOTA: al filtrar por 'translations__...' se hace JOIN contra la
        # tabla de traducciones (parler). Si una propiedad tiene varias
        # traducciones (es/en) que matchean el texto, aparecería duplicada
        # en el queryset sin el distinct() de aquí abajo.
        if data.get('q'):
            q = data['q']
...
def _serialize_property_detail(obj, request):
    ...
    detalle (quick view) que se abre al hacer doble click sobre una
    tarjeta del ``dxDataGrid`` de "Resultados" en la home.
    ...
    if not images:
        # Mismo fallback visual que _serialize_property_for_grid mientras
        # la propiedad no tenga fotos reales.
...
def property_detail_json(request, pk):
    """
    Endpoint JSON consumido por la ventana de detalle (quick view) del
    ``dxDataGrid`` de "Resultados" en la home: se llama al hacer doble
    click sobre una tarjeta, pasando su ``id``.

    Solo expone propiedades activas (mismo criterio que ``property_detail``
    y que el listado), para no filtrar por esta vía datos de propiedades
    despublicadas.
    """
